...
public:
    ...
    void delete_edge(const Node &a, const Node &b)
    {
        ...
        if (map_iter != Edges.end())
        {
            ...
            map_iter->second.erase(b);  // This line has the bug in original: map_iter->erase(b)
        ...
        if (map_iter != Edges.end())
        {
            ...
            map_iter->second.erase(a);  // This line has the bug in original: map_iter->erase(a)
            if (map_iter->second.empty())
                Edges.erase(map_iter);
        }
    }
...
// In the buggy version, this would cause a compilation error or undefined behavior
...
TEST(large_graph)
{
    ...
    for (int x = 0; x < 10; x++)
    {
        ...
        {
            int node = y * 10 + x;
            if (x + 1 < 10) g.add_edge(node, node + 1);
            if (y + 1 < 10) g.add_edge(node, node + 10);
        }
    }
